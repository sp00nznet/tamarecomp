...
def body(op):
    ...
    if m in ("SET", "RST"):
        # SET F,i is F <- F OR i. RST F,i is F <- F AND i -- it keeps the bits
        # named in i and clears the rest, which is the opposite of the obvious
        # reading. Getting this backwards is silent: the ROM runs, the screen
        # draws, and a comparison flag is quietly wrong 1500 instructions in.
        i = int(args[1], 16)
        out = []
        for bit, fl in ((1, "cf"), (2, "zf"), (4, "df")):
            if m == "SET" and (i & bit):
                out.append("t->%s = 1;" % fl)
            elif m == "RST" and not (i & bit):
                out.append("t->%s = 0;" % fl)
        if m == "SET" and (i & 8):
            out += ["t->if_delay = !t->iff;", "t->iff = 1;"]
        elif m == "RST" and not (i & 8):
            out.append("t->iff = 0;")
        return out or ["/* no flag bits affected */"]
...
/* Compiles to nothing unless the build asks for a trace. Recompiled code has
 * no per-instruction observation point -- that is rather the point of it -- so
 * differential testing against a reference core needs one put back by hand. */
#ifdef TAMA_TRACING
void tama_trace_hook(const tama_t *t, uint16_t addr);
#  define TAMA_TRACE(a) tama_trace_hook(t, (a))
#else
#  define TAMA_TRACE(a) ((void)0)
#endif
